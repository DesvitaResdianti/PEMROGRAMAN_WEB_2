...
$kode = $_GET['id'];
	$sql = "SELECT trx.*, tmp_trx.*, product.*, konsumen.*, kasir.* FROM trx, tmp_trx, product, konsumen, kasir WHERE trx.id_trx=tmp_trx.id_trx AND trx.id_kon=konsumen.id_kon AND tmp_trx.id_brg=product.id_brg AND trx.id_kasir = kasir.id_kasir  AND trx.id_trx='". $_GET['id'] ."'";
...
<head>
	...
	<link href="../foto/book.png" rel="icon" type="images/x-icon">
    ...
    <?php 
  $sqlx = mysqli_query($conn, "SELECT trx.*, tmp_trx.*, product.*, konsumen.*, kasir.* , disc.* FROM trx, tmp_trx, product, konsumen, kasir , disc WHERE trx.id_trx=tmp_trx.id_trx AND trx.id_kon=konsumen.id_kon AND tmp_trx.id_brg=product.id_brg AND trx.id_kasir = kasir.id_kasir and trx.id_discount=disc.id_disc AND trx.id_trx='". $_GET['id'] ."'");
  $res = mysqli_fetch_array($sqlx);
  ?>
				...
				<tbody>
					<?php
						...
						$sqltmp = "SELECT tmp_trx.*, product.* FROM tmp_trx, product WHERE tmp_trx.id_brg=product.id_brg
								AND tmp_trx.id_trx='$kode' ORDER BY product.nama ASC";
