...
<body>
    ...
    <?php 
  $sqlx = mysqli_query($conn, "SELECT trx.*, tmp_trx.*, product.*, konsumen.*, kasir.*, disc.*, status_transaksi.* FROM trx, tmp_trx, product, konsumen, kasir, disc, status_transaksi WHERE trx.id_trx=tmp_trx.id_trx AND trx.id_kon=konsumen.id_kon AND tmp_trx.id_brg=product.id_brg	AND trx.id_kasir = kasir.id_kasir  and trx.id_discount=disc.id_disc and trx.status_transaksi=status_transaksi.id_transaksi AND trx.id_trx='". $_GET['id'] ."'");
