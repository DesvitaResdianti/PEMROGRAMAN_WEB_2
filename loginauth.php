...
if(isset($_POST['login'])) {
    ...
    if($auth === 'kasir') {
        ...
        $query = "SELECT * from konsumen WHERE user_kon = '$user'";
        $exec = mysqli_query($conn,$query);
        if(mysqli_num_rows($exec) > 0) {
            $query = "SELECT * from konsumen WHERE password = '$pass'";
            $exec = mysqli_query($conn,$query);
            if(mysqli_num_rows($exec) > 0) {
                $res = mysqli_fetch_assoc($exec);
                $_SESSION['cust'] = $res['nama_kon'];
                $_SESSION['id_kon'] = $res['id_kon'];
               header("location: customer/barang.php");
            }else {
                echo "<script>alert('Password yang anda masukan tidak sesuai')</script>";
                echo "<script>document.location = 'loginauth.php'</script>";
            }
